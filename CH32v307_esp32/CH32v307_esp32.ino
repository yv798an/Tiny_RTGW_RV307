...
#include <ArduinoJson.h> // 1. 引入 ArduinoJson 库
...
// ================= 2. 手机热点与MQTT服务器配置 =================
const char* ssid         = "oppofindx8";     // 手机热点名称（须为2.4GHz）
const char* password     = "eiku8997";  // 手机热点密码
...
// MQTT 主题
const char* pub_topic    = "gateway/data/submit";    // 数据发布主题
const char* sub_topic    = "gateway/data/recommand"; // 数据订阅主题
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // 核心中转逻辑：把从云端 MQTT 收到的控制指令，直接通过串口转发
  Serial1.println(msg); 
}
...
void publishSensorData(int temp, int hum, int adc, int mv, int LED_state) {
  ...
  // 打印生成的 JSON 字符串到串口，方便调试
  Serial.print("Generated JSON: ");
  Serial.println(output);

  // 发布到 MQTT 主题
  if (client.publish(pub_topic, output.c_str())) {
...
void loop() {
  ...
  if (stringComplete) {
    ...
    if (inputString.length() > 0) {
      ...
      // 4. 解析 "$%d,%d,%d,%d,%d#" 格式的数据
      // 检查字符串是否以 '$' 开头，以 '#' 结尾
      if (inputString.startsWith("$") && inputString.endsWith("#")) {
        int val1, val2, val3, val4,va15;
        // 使用 sscanf 函数从字符串中提取四个整数
        // %*c 表示读取并忽略一个字符（即 '$' 和 '#'）
        int parsedItems = sscanf(inputString.c_str(), "$%d,%d,%d,%d,%d#", &val1, &val2, &val3, &val4,&va15);
        
        if (parsedItems == 5) {
          Serial.printf("Parsed data: %d, %d, %d, %d, %d\n", val1, val2, val3, val4, va15);
          // 调用新函数，将解析出的数据发布到 MQTT
