...
static void Light_Update(sensor_data_t *data)
{
    ...
    adc_value = ADC_Read_Avg(ADC_Channel_11,10);
    data->light_adc = adc_value;
    data->light_mv = (uint32_t)adc_value * 3300 / 4095;
}
...
void sensor_thread_entry(void *parameter)
{
    ...
    rt_memset(&sensor_data,0,sizeof(sensor_data_t));
    I2C1_Init();

    if(DHT20_Init())
    {
        rt_kprintf("DHT20 init failed!\n");
    }
    else
    {
        rt_kprintf("DHT20 ready\n");
    }
